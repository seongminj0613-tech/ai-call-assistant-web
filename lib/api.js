...
export const customerApi = {
  // 고객 목록 (통화 목록에서 전화번호 기준 그룹핑)
  list: (storeId) => api.get('/calls', { params: { store_id: storeId, limit: 500 } }),
};

// ============ 7. 캘린더 연동 API ============
export const calendarConnectApi = {
  // 연동된 캘린더 목록
  getConnections: () => api.get('/calendar/connections'),
  // OAuth URL 발급
  getOAuthUrl: (provider, redirectUri, state) =>
    api.post('/calendar/oauth/url', { provider, redirect_uri: redirectUri, state }),
  // 연동 해제
  disconnect: (provider) => api.delete(`/calendar/connections/${provider}`),
  // 특정 기간 일정 목록
  getEvents: (from, to) => api.get('/calendar/events', { params: { from, to, limit: 200 } }),
};

// ============ 8. 업로드 승인 API ============
export const pendingApi = {
  // 승인 대기 녹음 목록
  list: () => api.get('/calls/pending'),
  // 개별 승인
  approve: (callId) => api.post(`/calls/${callId}/approve`),
  // 개별 거절
  reject: (callId) => api.delete(`/calls/${callId}`),
};

// ============ 9. 키워드 API ============
export const keywordApi = {
  list: (storeId) => api.get(`/stores/${storeId}/keywords`),
  create: (storeId, keyword) => api.post(`/stores/${storeId}/keywords`, { keyword }),
  delete: (storeId, keywordId) => api.delete(`/stores/${storeId}/keywords/${keywordId}`),
};

// ============ 10. 설정 API ============
export const settingsApi = {
  getStore: (storeId) => api.get(`/stores/${storeId}`),
  updateStore: (storeId, data) => api.patch(`/stores/${storeId}`, data),
};
